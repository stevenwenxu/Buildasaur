...
public class HTTP {
    ...
    public func sendRequest(request: NSURLRequest, completion: Completion) -> NSURLSessionTask {
        ...
        Log.verbose("-------------------- Making request url: \(request.URL), headers: \(request.allHTTPHeaderFields), method: \(request.HTTPMethod), body: \(String.init(data: request.HTTPBody ?? NSData(), encoding: NSUTF8StringEncoding)))")
